<template>
  <div>
    <common-header :title="'采购管理'" :title1="'采购订单列表'"></common-header>
    <div class="page-content">
      <el-form :inline="true" :model="params" class="demo-form-inline"
        ...
        <table class="search-table" style="width:90%;font-size:12px;" cellspacing="10px" cellpadding="5px">
          ...
          <tr>
            <td>供应商名称</td>
            <td>
              <el-select v-model="params.supplier_id" placeholder="全部" filterable @change="search()" style="width:100%"
                         size="mini">
      ...
      <el-table :data="tableData" max-height="420" :summary-method="getSummaries" show-summary v-loading="loading"
        ...
        <el-table-column label="订单日期">
          <template slot-scope="scope">
            <span>{{scope.row.in_date.split(' ')[0]}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="s_name" label="供应商名称" width="100">
          <template slot-scope="scope">
            <span :title="scope.row.s_name">{{scope.row.s_name.slice(0, 5) + '...'}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="chmc_name" label="存货名称" width="100">
          <template slot-scope="scope">
        ...
        <el-table-column prop="total_price" label="含税金额" width="80">
        </el-table-column>
        <el-table-column prop="no_tax_price" label="未含税金额" width="90">
        </el-table-column>
        <el-table-column label="采购主体">
          <template slot-scope="scope">
        ...
        <el-table-column label="审核状态">
          <template slot-scope="scope">
            <span v-if="scope.row.check_status==0">未提交</span>
...
<script>
  ...
  import {list_all_purchase_select, list_purchase_orders} from '../../../api/index'
  ...
  export default {
    ...
    methods: {
      ...
      rowOption(row, event, column) {
        ...
        el.href = location.href.split('#')[0]+'#/admin_view/purchasing_orderDesc/' + row.order_id; //url 是你得到的连接
      ...
      getData() {
        ...
        var _this = this;
        _this.loading = true;
        list_purchase_orders(this.params).then(function (res) {
      ...
      handleCheck(order_id) {
        this.$router.push({'name': 'purchasing_orderDesc', params: {'purchaseOrderId': order_id}})
      },
      exportExcel() {
        ...
        this.excelData.forEach(function (item) {
          ...
          obj.in_num = item.in_num;
          obj.in_date = item.in_date;
          obj.s_name = item.s_name;
        ...
        for (var i = 0; i < this.newData.length; i++) {
          ...
          if (this.newData[i].invoice_status == 0) {
            this.newData[i].invoice_status = '未开票';
          } else {
            this.newData[i].invoice_status = '已开票';
          }
        }

        var path = '../../../vendor/Export2Excel.js';
        var theader = ['订单编号', '订单日期', '供应商名称', '存货名称', '规格型号', '数量', '含税金额', '未含税金额', '销售主体', '付款状态', '发货状态', '开票状态', '备注'];
        var filterVal = ['in_num', 'in_date', 's_name', 'chmc_name', 'unit', 'number', 'total_price', 'no_tax_price', 'entity_name', 'pay', 'receive', 'invoice_status', 'remarks'];
        var tableData = this.newData;
        var excelName = '采购订单列表';
