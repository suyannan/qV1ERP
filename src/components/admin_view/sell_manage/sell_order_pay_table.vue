<template>
  <div class="self-table">
    <h4>收款单</h4>
    <div>
      <el-table :data="order_pay_list" border max-height="450"  style="width: 100%; margin-top: 20px" :summary-method="getSummaries"
                show-summary>
        <el-table-column type="index" width="50">
        </el-table-column>
        <el-table-column prop="pay_date" label="收款日期"></el-table-column>
        <el-table-column prop="entity_name" label="收款公司名称" width="175"></el-table-column>
        <el-table-column prop="price" label="实际收款金额"></el-table-column>
        <el-table-column prop="rebate_price" label="使用返利金额"></el-table-column>
          ...
          <template slot-scope="scope">
            ...
            <div v-if="order_check_status==2">
              ...
              <div v-if="is_super==0">
                ...
                <!--同时有采购录入删除权限和采购审核权限-->
                <div v-if="sell_pay_enter==1&&sell_pay_check==1">
                ...
                <!--只有收款单录入删除权限，没有收款单审核权限-->
                <div v-if="sell_pay_enter==1&&sell_pay_check==0">
                  <!--有录入权限，在执行录入功能的时候，需要判断订单是不是别人录入的订单，录入人员无法编辑别人录入的订单-->
                  <!--是自己录入的订单时-->
                  <div v-if="userId==scope.row.create_user_id">
                ...
                <!--没有收款单录入删除权限，只有收款单审核权限-->
                <div v-if="sell_pay_enter==0&&sell_pay_check==1">
                  <div v-if="scope.row.check_status==0">
                ...
                <div v-if="sell_pay_enter==0&&sell_pay_check==0">
                  <div v-if="scope.row.check_status==0||scope.row.check_status==3">
                    <el-button v-if="scope.row.check_status==0" size="mini" type="primary" disabled>未提交审核</el-button>
                    <el-button v-if="scope.row.check_status==3" size="mini" type="primary" disabled>审核不通过</el-button>
    ...
    <div class="detail-add-btn">
      <!--只有收款单录入人员和订单状态是2时才可以增加收款单-->
      <div v-if="order_check_status==2&&sell_pay_enter==1">
        <el-button type="primary" @click="add()" size="small">增加收款单</el-button>
      </div>
    </div>
  </div>
</template>
...
<script>
  import {delete_order_pay,check_sell_order_pay,submit_check_sell_order_pay} from '../../../api/index'
    ...
    methods:{
      ...
      orderDelete(row){
        ...
        this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
          ...
          delete_order_pay({'token':window.sessionStorage.getItem('session'),'id':row.id}).then(function (res) {
      ...
      add(){
        this.$router.push({'name':'add_pay_order',params:{'type':'add','sellOrderId':this.sellOrderId,'id':'add'}})
      },
      orderEdit(row){
        this.$router.push({'name':'add_pay_order',params:{'type':'edit','sellOrderId':this.sellOrderId,'id':row.id}})
      },
      /*提交审核*/
      upCheck(row){
        var _this=this;
        submit_check_sell_order_pay({'token':window.sessionStorage.getItem('session'),'id':row.id}).then(function (res) {
          if(res.data.errcode==0){
            _this.$message({
              message: '提交成功',
              type: 'success'
      ...
      upPassOrNot(obj,id){
        var _this=this;
        check_sell_order_pay(obj).then(function (res) {
          if(res.data.errcode==0){
            _this.$message({
              type: 'success',
              message: '提交成功'
